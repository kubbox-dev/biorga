<?php
	...
	if ( ! function_exists( 'breadcrumbs' ) ) {
		function breadcrumbs ( $list_style = 'ol', $list_id = 'breadcrumbs', $list_class = 'breadcrumbs', $active_class = 'active', $echo = false ) {
			// Get text domain for translations
			$theme = wp_get_theme();
			$text_domain =  $theme->get( 'biorga' );

			// Open list
			$breadcrumb = '<nav aria-label="You are here:" role="navigation" class="nav_breadcrumbs">';
			$breadcrumb .= '<' . $list_style . ' id="' . $list_id . '" class="' . $list_class . '">';

			// Front page
			if ( is_front_page() ) {
				$breadcrumb .= '<li class="' . $active_class . '">' . __( 'Inicio', 'biorga' ). '</li>';
			} else {
				$breadcrumb .= '<li><a href="' . home_url() . '">' . __( 'Inicio', 'biorga' ) . '</a></li>';
			}
